...
- **Unreal Engine 5.7**(v0.1 仅在 5.7 上端到端验证;5.5 / 5.6 兼容是 v0.2 目标;5.4 及以下不支持)
...
UE 5.7 的 Live Coding 对**已有 `UFUNCTION` 的函数体实现**(`.cpp` 内部逻辑)改动能热更新。例如:改 `ReadDataTable` 里的解析逻辑、改 Session Machine 某个状态的 tick 实现 —— 都可以直接 Ctrl+Alt+F11 热更。
...
- Windows UE 5.7 编译(v0.1 唯一受支持版本)
- clang-format 检查(v0.1 advisory,等作者本地装上 clang-format 后转 blocking)
- 文档 lint / forbidden-brand / api-key 检查(强制)

UE 5.5 / 5.6 matrix 待 v0.2 验证后扩展。
